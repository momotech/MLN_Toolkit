...
---  网络请求工具类
---
---@note  网络使用的cache策略请参考[CachePolicy](CachePolicy.html)
---		网络请求的加密策略请参考[EncType](EncType.html)
---		网络响应体中相关key请参考[ResponseKey](ResponseKey.html)
---		网络错误信息中相关key请参考[ErrorKey](ErrorKey.html)
...
---
---  配置根地址，可以避免每次都拼接重复的基础地址,[自定义设置BaseUrl的实现](../docs/自定义设置BaseUrl的实现.html)
---
...
---@return Http 返回http对象本身
---@note  具体的cache策略请参考[CachePolicy](CachePolicy.html)
...
---@param url string  url地址，如果设置了BaseURL可以只填写path
---@param param Map  请求使用到的所有参数，可以是Map
---@param progressCallback function  下载进度回调，格式为：
---		```
...
---		end
---		```
---@param finishedCallback function  网络结果回调方法，格式为：
---		```
---		function(success, response, error)
---			--- success为boolean，请求是否成功
---			--- response为Map，网络响应数据
...
---		```
---@return Http 返回http对象本身
function _class:download(url, param, progressCallback, finishedCallback)
	return self
end
