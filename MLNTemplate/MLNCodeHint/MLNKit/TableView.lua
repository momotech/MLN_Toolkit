...
---
---@note  当cell的样式一致时，使用默认初始化方法即可，当cell样式较多时，要采用复用ID的形式进行初始化和赋值操作。下拉刷新和上拉加载功能两端需要做桥接[RefreshHandler](../docs/iOS_ScrollView_RefreshHandler桥接步骤.html)，另外，iOS默认展示滚动指示条，android默认不展示
...
---
---
---  设置TableView的内容内边距，主要用来处理需要在内容范围需要压缩的场景
---
---@param top number  上
...
---		 ``` 
---		 function(number top,number right,number bottom,number left) 
---		 	 ---top:上边距 
---		 	 ---right:右边距 
---		 	 ---bottom:底边距 
---		 	 ---left:左边距 
